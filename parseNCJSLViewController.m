...
@interface parseNCJSLViewController ()

@end

@implementation parseNCJSLViewController
...
-(NSMutableArray *) parseRecvData :(NSString*) Translatecode :(NSString *) Parsedata//要传入的交易代码  和要解析的报文
{
    ...
    NSMutableArray *sumjsttjsArray=[[NSMutableArray alloc] init];  //及时妥投件数
    ...
    for(int getcount=0;getcount<[[outpub getValue:@"COLL":0]intValue];getcount++){
        ...
        [sumjsttjsArray addObject:[outpub getValue:@"N_DRTTJS" :getcount]];
    ...
    //这里是添加一行数组显示合计
    float jsttlaverge =[[[sumjkjsArray copy] valueForKeyPath:@"@sum.floatValue"]floatValue ];
    float jsttlaverge2=[[[sumztjjsArray copy] valueForKeyPath:@"@sum.floatValue"]floatValue];
    float jsttlaverge3=[[[sumjsttjsArray copy] valueForKeyPath:@"@sum.floatValue"]floatValue];
    float jsttlzhanbi=jsttlaverge3/(jsttlaverge-jsttlaverge2);
    //NSLog(@"1 [%f] 2 [%f] 3 [%f]占比[%0.4f]",jsttlaverge,jsttlaverge2,jsttlaverge3,jsttlzhanbi);
    ...
    //NSLog(@"1 [%f] 2 [%f] 3 [%f]占比[%0.4f]",ljttlaverge,ljttlaverge2,ljttlaverge3,ljttlzhanbi);
    ...
    [totalsum addObject: [NSString stringWithFormat:@"%@",[[sumjsttjsArray copy] valueForKeyPath:@"@sum.floatValue"]]];
    
    [totalsum addObject: [NSString stringWithFormat:@"%0.2f%@",jsttlzhanbi*100,@"%"]]; //占比
    ...
    [adicgetdata addObject:@"及时妥投件数"];
    [adicgetdata addObject:@"及时妥投率"];

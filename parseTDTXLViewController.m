...
#import "parseTDTXLViewController.h"

@interface parseTDTXLViewController ()

@end

@implementation parseTDTXLViewController
...
-(NSMutableArray *) parseRecvData :(NSString*) Translatecode :(NSString *) Parsedata//要传入的交易代码  和要解析的报文
{
    ...
    for(int getcount=0;getcount<[[outpub getValue:@"COLL":0]intValue];getcount++){
        ...
        [[outputgetdata objectAtIndex:getcount] addObject:[outpub getValue:@"V_YGXM" :getcount]];
        [[outputgetdata objectAtIndex:getcount] addObject:[outpub getValue:@"V_RYZW" :getcount]];
        [[outputgetdata objectAtIndex:getcount] addObject:[outpub getValue:@"V_LXDH" :getcount]];
    ...
    [adicgetdata addObject:@"员工姓名"];
    [adicgetdata addObject:@"职位"];
    [adicgetdata addObject:@"联系电话"];

...
@interface parseZDCSTTLViewController ()

@end

@implementation parseZDCSTTLViewController
...
-(NSMutableArray *) parseRecvData :(NSString*) Translatecode :(NSString *) Parsedata//要传入的交易代码  和要解析的报文
{
    ...
    for(int getcount=0;getcount<[[outpub getValue:@"COLL":0]intValue];getcount++){
        ...
        [[outputgetdata objectAtIndex:getcount] addObject:[outpub getValue:@"RATE" :getcount]];
        
        [[outputgetdata objectAtIndex:getcount] addObject:[outpub getValue:@"SRTTJS" :getcount]];
        [[outputgetdata objectAtIndex:getcount] addObject:[outpub getValue:@"RATE1" :getcount]];
        [[outputgetdata objectAtIndex:getcount] addObject:[outpub getValue:@"N_LLTTS" :getcount]];
        [[outputgetdata objectAtIndex:getcount] addObject:[outpub getValue:@"RATE2" :getcount]];
